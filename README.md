...
- SSH 连接复用：同一主机多会话共享底层 TCP 连接，减少握手开销
- Jump Host 跳板机支持：通过中间主机跳转连接目标机器
- 端口转发：支持本地转发（Local Forward）和远程转发（Remote Forward）
...
- **内置密钥生成**：支持 Ed25519 和 RSA 密钥对生成
- **GitHub 更新检查**：设置页一键检查 GitHub Releases 新版本
- **自动更新**：内置版本检查和更新器
- **强调色预设**：6 种强调色（天蓝/翡翠/紫罗兰/琥珀/玫瑰/青色），深浅模式独立配色
- **多语言**：中文/英文，首次启动默认中文
